    ...
    view! {
        <div class="animate-fade-in">
            <div class="flex items-center justify-between mb-6">
                ...
                <button on:click=move|_| { form_name.set(String::new()); new_key_data.set(None); show_form.set(true); }
                    class="px-4 py-2 text-sm font-medium rounded-lg text-white bg-accent hover:bg-accent-hover transition-all duration-150 flex items-center gap-2">
            ...
            {move || new_key_data.get().map(|nk| {
                ...
                view! {
                    <div class="mb-6 p-4 rounded-xl bg-success-bg border border-success animate-scale-in">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <p class="text-sm font-semibold text-success">"API Key Created — Copy it now!"</p>
                        </div>
                        <p class="text-xs text-success/70 mb-2">"This key will not be shown again."</p>
                        <div class="flex gap-2">
                            <code class="flex-1 px-3 py-2 rounded-lg bg-[#0d1117] text-sm font-mono text-accent break-all select-all">{full_key.clone()}</code>
                            <button on:click=move|_| {
                                let key = full_key.clone();
                                spawn_local(async move {
                                    let window = web_sys::window().unwrap();
                                    // Clipboard accessible via navigator.clipboard in secure contexts
                                    let _ = window.navigator().clipboard().write_text(&key);

                                });
                            }
                                class="px-3 py-2 text-xs font-medium rounded-lg bg-surface-active text-primary hover:bg-border transition-all duration-150">
                                "Copy"
                            </button>
                        </div>
                    </div>
            ...
            {move || show_form.get().then(|| {
                view! {
                    <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 animate-fade-in"
                        ...
                        <div class="bg-surface-alt border border-surface rounded-xl w-full max-w-md mx-4 shadow-2xl animate-scale-in"
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex gap-3 justify-end mt-6 pt-4 border-t border-surface">
                                    ...
                                    <button on:click=move|_|create() disabled=saving.get()
                                        class="px-4 py-2 text-sm font-medium rounded-lg text-white bg-accent hover:bg-accent-hover disabled:opacity-50 transition-all duration-150">
                                        "Create"
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 animate-fade-in"
                        ...
                        <div class="bg-surface-alt border border-surface rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl animate-scale-in"
                            ...
                            <div class="flex items-start gap-3 mb-4">
                                ...
                                <div>
                                    <h3 class="text-base font-semibold text-primary">{format!("Revoke \"{}\"?", name)}</h3>
                            ...
                            <div class="flex gap-2 justify-end">
                                ...
                                <button on:click=move|_|do_delete(id.clone())
                                    class="px-4 py-2 text-sm font-medium rounded-lg text-white bg-danger hover:bg-red-600 transition-all duration-150">"Revoke"</button>
                            </div>
                        </div>
                    </div>
                }
                    ...
                    <div class="bg-surface-alt border border-surface rounded-xl overflow-hidden animate-fade-in-up">
                        <table class="w-full">
                            <thead><tr class="bg-surface-hover">
                                ...
                                <th class="text-left px-4 py-3 text-xs font-semibold text-secondary uppercase tracking-wider">"Key Prefix"</th>
                                <th class="text-left px-4 py-3 text-xs font-semibold text-secondary uppercase tracking-wider">"Created"</th>
                                ...
                                {ks.into_iter().map(|k| {
                                    ...
                                    view! {
                                        <tr class="hover:bg-surface-hover/50 transition-colors duration-100">
                                            ...
                                            <td class="px-4 py-3"><code class="text-sm font-mono text-accent bg-surface-active px-2 py-0.5 rounded">{k.key_prefix}...</code></td>
                                            <td class="px-4 py-3 text-sm text-secondary">{k.created_at[..10].to_string()}</td>
                                            <td class="px-4 py-3">{
                                                if k.enabled {
                                                    ...
                                                    view! { <span class="inline-flex items-center gap-1 text-xs text-muted"><span class="w-1.5 h-1.5 rounded-full bg-muted"></span>"Revoked"</span> }
                                                }
                                            }</td>
                                            <td class="px-4 py-3 text-right">
                                                <button on:click=move|_|delete_id.set(Some(kid.clone()))
                                                    class="px-2.5 py-1.5 text-xs font-medium rounded-lg text-danger border border-danger/30 hover:bg-danger-bg transition-all duration-150">"Revoke"</button>
                                            </td>
                                        </tr>
                                    }
                                }).collect::<Vec<_>>()}
                        ...
                        {keys.with(|k| k.is_empty()).then(|| view! { <div class="text-center py-12 text-muted text-sm">"No API keys yet."</div> })}
                    </div>
                }
            })}
        </div>
