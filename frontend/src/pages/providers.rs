...
pub fn Providers() -> impl IntoView {
    ...
    let handle_test_model = move |provider_id: String, model: String| {
        ...
        if testing_model.with(|t| t.as_deref() == Some(&key)) { return; }
        ...
        spawn_local({
            ...
            async move {
                ...
                match result {
                    Ok(r) => { model_test_results.update(|m| { m.insert(key.clone(), r); }); }
                    Err(e) => {
            ...
            view! {
                <div class="mb-4">
                    ...
                    <button on:click=move|_| do_oauth_login(bt.clone())
                        class="btn-base w-full px-4 py-3 text-sm rounded-lg bg-accent text-white hover:bg-accent-hover">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"/></svg>
                        "Login with "{pn.clone()}
                    </button>
                </div>
            }.into_any()
            ...
            els.push(Box::new(move || {
                ...
                view! {
                    <div class="mb-4">
                        ...
                        <button on:click=move|_| do_device_code(dc_ptype.clone()) disabled=is_dev
                            class="btn-base w-full px-4 py-2 text-sm rounded-lg border border-accent/30 text-accent hover:bg-accent-bg">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                            "Device Code"
                        </button>
                        {dev_data.map(|dd| view! {
                            <div class="mt-3 p-3 rounded-lg bg-surface-2 border border-border-subtle">
                                <p class="text-xs text-secondary mb-1">"Open" <a href=dd.verification_uri.clone() target="_blank" class="text-accent underline hover:text-accent-hover">{dd.verification_uri.clone()}</a></p>
                                <p class="text-xs text-secondary mb-1">"Enter code:"</p>
                                <p class="text-lg font-mono font-bold text-primary tracking-wider text-center py-2.5 bg-bg rounded-lg border border-border-subtle">{dd.user_code.clone()}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex gap-2">
                            <input type="password" prop:value=move || cookie_val.get()
                                ...
                                class="flex-1 px-3 py-2 bg-surface-2 border border-border-subtle rounded-lg text-sm text-primary placeholder-muted focus:border-accent focus:outline-none transition-colors"/>
                            <button on:click=move|_| do_import_token(im_ptype.clone(), cookie_val.get())
                                class="btn-base px-3 py-2 text-sm rounded-lg bg-accent text-white hover:bg-accent-hover">"Import"</button>
                        </div>
                    </div>
                }.into_any()
            }));
                ...
                <div class="mb-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button on:click=move|_| do_import_token(wc_ptype.clone(), cookie_val.get())
                            class="btn-base px-3 py-2 text-sm rounded-lg bg-[#db6b9a] text-white hover:bg-[#c05d82]">"Import Cookie"</button>
                    </div>
                </div>
            }.into_any()
        }));
    ...
    view! {
        <div class="animate-fade-in">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-primary font-display tracking-tight">"Providers"</h1>
            ...
            {move || (!error.get().is_empty()).then(|| view! {
                <p class="mb-4 p-3 rounded-lg bg-danger-bg text-danger text-sm border border-danger/20">{error.get()}</p>
            })}
            ...
            {move || match oauth_flow.get() {
                OAuthFlowState::Authorizing { ref provider } => Some(view! {
                    <div class="mb-4 p-3 rounded-lg bg-warning-bg border border-warning/30 text-sm text-warning flex items-center gap-2">
                        ...
                        "Waiting for OAuth login for " {provider.clone()} "..."
                    </div>
                }),
                OAuthFlowState::WaitingDevice { ref provider, .. } => Some(view! {
                    <div class="mb-4 p-3 rounded-lg bg-warning-bg border border-warning/30 text-sm text-warning flex items-center gap-2">
                        <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                        "Waiting for device code for " {provider.clone()} "..."
                    </div>
                }),
                OAuthFlowState::Done => Some(view! {
                    <div class="mb-4 p-3 rounded-lg bg-success-bg border border-success/30 text-sm text-success flex items-center gap-2">
                        <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                ...
                OAuthFlowState::Error(ref msg) => Some(view! {
                    <div class="mb-4 p-3 rounded-lg bg-danger-bg border border-danger/30 text-sm text-danger flex items-center gap-2">
                        <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.072 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
                        {msg.clone()}
                    </div>
                }),
                _ => None,
                        ...
                        <div class="bg-surface border border-border-subtle rounded-xl p-6 w-full max-w-md mx-4 shadow-[var(--shadow-elev)] animate-scale-in"
                            ...
                            <div class="flex items-start gap-3 mb-4">
                                <div class="w-10 h-10 rounded-full bg-danger-bg flex items-center justify-center shrink-0">
                                    <svg class="w-5 h-5 text-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.072 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
                                </div>
                                <div>
                                    <h3 class="text-base font-semibold text-primary font-display tracking-tight">{format!("Delete \"{}\"?", name)}</h3>
            ...
            {move || (show_form.get() && provider_types_loaded.get()).then(|| {
                ...
                view! {
                    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-start justify-center pt-[10vh] z-50 animate-fade-in"
                        on:click=move|_| show_form.set(false)>
                        <div class="bg-surface border border-border-subtle rounded-xl w-full max-w-lg mx-4 max-h-[80vh] overflow-y-auto shadow-[var(--shadow-elev)] animate-scale-in"
                            on:click=move|ev| ev.stop_propagation()>
                            <div class="flex items-center justify-between px-6 py-4 border-b border-border-subtle">
                                <h2 class="text-base font-semibold text-primary font-display tracking-tight">{if is_edit { "Edit Provider" } else { "Add Provider" }}</h2>
                                    ...
                                    if is_free_type.get() {
                                        view! {
                                            <div class="mb-4 opacity-50 pointer-events-none">
                                                ...
                                                <input type="text" disabled=true value="(no key needed)" class="w-full px-3 py-2 bg-surface-2 border border-border-subtle rounded-lg text-sm text-muted"/>
                                            </div>
                                        }.into_view()
                                        ...
                                        view! { {render_oauth_form(pt.clone(), pn).into_iter().map(|f| f()).collect::<Vec<_>>()} }.into_view()
                                    } else if is_webcookie_selected.get() {
                                        let sel = selected_type_info.get();
                                        let pn = sel.as_ref().map(|i| i.display_name.clone()).unwrap_or_default();
                                        let pt = form_type.get();
                                        view! { {render_webcookie_form(pt.clone(), pn).into_iter().map(|f| f()).collect::<Vec<_>>()} }.into_view()
                                    } else {
                                        view! {
                                            <div class="mb-4">
                                                <label class="block text-xs text-secondary mb-1.5 font-medium">"API Key"</label>
                                    ...
                                    if is_free || is_oauth || is_wc {
                                        view! {
                                            <div class="mb-4 opacity-50 pointer-events-none">
                                                ...
                                                <input type="text" disabled=true value={if is_free { "(hardcoded)" } else { "(auto)" }} class="w-full px-3 py-2 bg-surface-2 border border-border-subtle rounded-lg text-sm text-muted"/>
                                            </div>
                                        }.into_view()
                                    } else {
                                        view! {
                                            <div class="mb-4">
                                                <label class="block text-xs text-secondary mb-1.5 font-medium">"Base URL"</label>
                                                <input type="text" prop:value=form_url.get() placeholder="https://api.example.com/v1"
                                        ...
                                        if !is_oauth_selected.get() && !is_webcookie_selected.get() {
                                            view! {
                                                <button on:click=move|_| do_save() disabled=saving.get()
                                                    class="btn-base px-4 py-2 text-sm rounded-lg bg-accent hover:bg-accent-hover text-white">{if saving.get() { "Saving..." } else { "Save" }}</button>
                                            }.into_view()
                                        } else {
                ...
                if secs.is_empty() {
                    view! {
                        <div class="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <p class="text-muted text-xs mt-1">"Add a provider to get started"</p>
                        </div>
                    }.into_view()
                } else {
                    view! {
                                ...
                                view! {
                                    <section>
                                        <div class="flex items-center gap-3 mb-4">
                                            ...
                                            <h2 class="text-base font-semibold text-primary font-display tracking-tight">{section_title.clone()}</h2>
